...
class FPA(nn.Module):
    def __init__(self, in_channels, out_channels, is_norm=True, is_spec=False):
        ...
        self.down2_2 = nn.Sequential(
            ...
            nn.InstanceNorm2d(out_channels) if is_norm else nn.Identity(),
            nn.SiLU(),
        )
...
class DecodeBlock(nn.Module):
    def __init__(self, ui_channels, xi_channels, out_channels, is_norm=True, is_spec=False, is_cbam=True, cbam_kernel=7):
        ...
        self.u_conv = nn.Upsample(scale_factor=2, mode="bilinear", align_corners=False)
...
class UNetGenerator(nn.Module):
    def __init__(self, in_channels, channels, out_channels, is_norm=True, is_spec=False, is_cbam=True):
        ...
        self.conv = nn.Sequential(
            ...
            nn.InstanceNorm2d(channels) if is_norm else nn.Identity(),
            nn.SiLU(),
        )
        ...
        self.encode5 = nn.Sequential(
            ResBlock(4 * channels, 8 * channels, stride=2, is_norm=is_norm, is_spec=is_spec),
        )

        self.center = nn.Sequential(
            FPA(8 * channels, 1 * channels, is_norm=is_norm, is_spec=is_spec),
            nn.AvgPool2d(2, 2),
        )

        self.decode5 = DecodeBlock(1 * channels, 8 * channels, 1 * channels, is_norm=is_norm, is_spec=is_spec, is_cbam=is_cbam, cbam_kernel=9)
        self.decode4 = DecodeBlock(1 * channels, 4 * channels, 1 * channels, is_norm=is_norm, is_spec=is_spec, is_cbam=is_cbam, cbam_kernel=7)
        self.decode3 = DecodeBlock(1 * channels, 2 * channels, 1 * channels, is_norm=is_norm, is_spec=is_spec, is_cbam=is_cbam, cbam_kernel=5)
        ...
        self.logit = nn.Sequential(
            ...
            # nn.SiLU(),
            conv(4 * channels, out_channels, 1, stride=1, padding=0),
        )
    ...
    def forward_normal(self, x, extract_features=False):
        e1 = self.conv(x)
        e2 = self.encode2(e1)
        e3 = self.encode3(e2)
        e4 = self.encode4(e3)
    ...
    def forward_check(self, x, extract_features=False):
        e1 = self.conv(x)
    ...
    def forward(self, x, extract_features=False, is_check=False):
        if is_check:
            return self.forward_check(x, extract_features=extract_features)
        else:
            return self.forward_normal(x, extract_features=extract_features)
...
class ConvDiscriminator(nn.Module):
    def __init__(self, in_channels, channels, is_norm=True, is_spec=False, is_cbam=True):
        ...
        self.conv3 = nn.Sequential(
            conv(4 * channels, 8 * channels, 4, stride=2, padding=1),
        ...
        self.conv4 = nn.Sequential(
            conv(8 * channels, 8 * channels, 4, stride=2, padding=1),
            nn.InstanceNorm2d(8 * channels) if is_norm else nn.Identity(),
            nn.LeakyReLU(0.2),
            conv(8 * channels, 8 * channels, 3, stride=1, padding=1),
            nn.InstanceNorm2d(8 * channels) if is_norm else nn.Identity(),
...
class PatchNCELoss(nn.Module):
    ...
    def forward(self, feat_q, feat_k):
        ...
        labels = labels.unsqueeze(0).expand(B, S).reshape(-1) # -> [B * S]
...
if __name__ == "__main__":
    ...
    batch_size = 4
    gen = UNetGenerator(1, 32, 1, is_norm=True, is_spec=True, is_cbam=True)
    dis = ConvDiscriminator(1, 32, is_norm=False, is_spec=True, is_cbam=True)
    gen.apply(init_weights)
    dis.apply(init_weights)

    x = torch.randn(batch_size, 1, 72, 5760)
    y = gen(x, is_check=True)
    r = dis(y, is_check=True)
    print(x.shape)
    print(y.shape)
    print(r.shape)
