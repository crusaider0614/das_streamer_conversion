...
def train(rank, world_size, CF):
    ...
    D_A = get_dis_model(CF, True).to(rank)
    ...
    if is_parallel:
        ...
        D_A = DDP(D_A, device_ids=[rank], find_unused_parameters=True)
    ...
    optimizer_D_A = optim.Adam(D_A.parameters(), lr=CF.TRAIN.DIS_LR, betas=(CF.TRAIN.BETA1, CF.TRAIN.BETA2))
    ...
    lr_scheduler_D_A = optim.lr_scheduler.ExponentialLR(optimizer_D_A, gamma=1.0)
    ...
    if CF.PRETRAIN.LOAD:
        ...
        if is_parallel:
            ...
            D_A.module.load_state_dict(state["D_A"])
        ...
        if CF.PRETRAIN.LOAD_OPTIMIZER:
            ...
            lr_scheduler_D_A.load_state_dict(state["lr_scheduler_D_A"])
        ...
        D_A_losses = state["D_A_loss"]
        G_A_losses = state["G_A_loss"]
    ...
    avg_D_A_loss = ValueTracker(ema_coeff)
    avg_G_A_loss = ValueTracker(ema_coeff)
    ...
    for i_epoch in range(CF.TRAIN.BEGIN_EPOCH, CF.TRAIN.END_EPOCH):
        if rank == 0:
            ...
            lr_D_A = lr_scheduler_D_A.get_last_lr()
            lr_D_B = lr_scheduler_D_B.get_last_lr()
            print(f"epoch {i_epoch + 1}: learning rate = {lr_G[0]} {lr_D_A[0]} {lr_D_B[0]}")
        start_time = time.time()
        avg_identity_loss.initialize()
        avg_cycle_loss.initialize()
        avg_G_loss.initialize()
        avg_D_A_loss.initialize()
        avg_G_A_loss.initialize()
        ...
        for i_iter in range(CF.TRAIN.ITER_PER_EPOCH):
            for i_batch, (real_A_images, real_B_images) in enumerate(zip(A_loader, B_loader)):
                ...
                with autocast(device_type="cuda"):
                    ...
                    GAN_loss = -(fake_A_logits.mean() + fake_B_logits.mean())
                ...
                avg_G_A_loss.feed(fake_A_logits.detach().mean().item())
                ...
                with autocast(device_type="cuda"):
                    real_A_logits = D_A(real_A_images.detach())
                    fake_A_logits = D_A(fake_A_images.detach())

                    # D_A_loss = adversarial_criterion(fake_A_logits, fake_labels) + adversarial_criterion(real_A_logits, real_labels)
                    D_A_loss = nn.ReLU()(1.0 + fake_A_logits).mean() + nn.ReLU()(1.0 - real_A_logits).mean()
                ...
                avg_D_A_loss.feed(real_A_logits.detach().mean().item())
                ...
                if rank == 0 and ((i_batch + 1) % 10 == 0 or (i_batch + 1) == n_batch):
                    print("epoch: {:4}, batch: {:4}, identity_loss: {:8.3e}, cycle_loss: {:8.3e}, G_loss: {:8.3e}, A_score: {:7.4f}, {:7.4f}, B_score: {:7.4f}, {:7.4f}".format(
                        i_epoch + 1,
                        i_batch + 1,
        ...
        if rank == 0:
            if True:
                ...
                D_A_losses.append(avg_D_A_loss.val())
                G_A_losses.append(avg_G_A_loss.val())
                ...
                state = {
                    ...
                    "D_A": D_A.module.state_dict() if is_parallel else D_A.state_dict(),
...
if __name__ == "__main__":
    main("pohang_shore_das_str.yaml")
